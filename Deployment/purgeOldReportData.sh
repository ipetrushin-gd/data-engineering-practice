...
echo "=====> INFO: Purging reports older than $past_date from $report_path....."

today=`date +'%s'`
hdfs dfs -ls $dfReportPath/ | grep "^d" | while read line ; do
...
hdfs dfs -ls $sqlReportPath/ | grep "^d" | while read line ; do
dir_date=$(echo ${line} | awk '{print $6}')
difference=$(( ( ${today} - $(date -d ${dir_date} +%s) ) / ( 24*60*60 ) ))
filePath=$(echo ${line} | awk '{print $8}')
...
hdfs dfs -ls $dsReportPath/ | grep "^d" | while read line ; do
dir_date=$(echo ${line} | awk '{print $6}')
difference=$(( ( ${today} - $(date -d ${dir_date} +%s) ) / ( 24*60*60 ) ))
filePath=$(echo ${line} | awk '{print $8}')
